...
class PrecisionOverClasses(precision_recall.Precision):
    def __init__(self, num_classes: int = 1, threshold: float = 0.5, average: str = 'micro', mdmc_average: Optional[str] = None, #multilabel: bool = False,
            compute_on_step: bool = True, dist_sync_on_step: bool = False, process_group: Optional[Any] = None, ):
        super().__init__(num_classes=num_classes, threshold=threshold, average=average, mdmc_average = mdmc_average, #multilabel=multilabel,
    ...
    def compute(self):
        ...
        return precision_recall._precision_compute(tp, fp, fn, self.average, self.mdmc_reduce)

class RecallOverClasse(precision_recall.Recall):
    def __init__(self, num_classes: int = 1, threshold: float = 0.5, average: str = 'micro', mdmc_average: Optional[str] = None,  #multilabel: bool = False,
            compute_on_step: bool = True, dist_sync_on_step: bool = False, process_group: Optional[Any] = None, ):
        super().__init__(num_classes=num_classes, threshold=threshold, average=average, mdmc_average = mdmc_average,  #multilabel=multilabel,
                         compute_on_step=compute_on_step, dist_sync_on_step=dist_sync_on_step, process_group=process_group)
    def compute(self):
        ...
        tp, fp, _, fn = self._get_final_stats()
        return precision_recall._recall_compute(tp, fp, fn, self.average, self.mdmc_reduce)
...
class AccuracyStages(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        for s in range(self.num_stages):
            preds_stage, target_stage, _ = _input_format_classification(preds[s], target)
            assert preds_stage.shape == target_stage.shape

            s_correct = getattr(self, f"S{s + 1}_correct")
            s_correct += torch.sum(preds_stage * target_stage)
