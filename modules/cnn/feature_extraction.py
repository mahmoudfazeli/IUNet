...
class FeatureExtraction(LightningModule):
    ...
    def init_metrics(self):
        ...
        if self.num_tasks == 2:
            self.train_acc_tool = torchmetrics.Accuracy()
            self.val_acc_tool = torchmetrics.Accuracy()
            self.val_f1_tool = torchmetrics.FBeta(num_classes=7)
            self.train_f1_tool = torchmetrics.FBeta(num_classes=7)
    ...
    def __tb_dataloader(self, split=None):
        dataset = self.dataset.data[split]
        worker=2
        if split == "val" or split == "test":
            should_shuffle = False
    ...
    def train_dataloader(self):
        ...
        tb_dataloader = self.__tb_dataloader(split="train")
        dataiter = iter(tb_dataloader)
        images, label1, label2 = dataiter.next()
        img_grid = torchvision.utils.make_grid(images)
        #print(img_grid)
        #self.matplotlib_imshow(img_grid)
        writer = SummaryWriter('tb_photo')
        writer.add_image('Cholec80_train_images', img_grid)
        return dataloader #, images, label1, label2
    ...
    def val_dataloader(self):
        ...
        logging.info("validation data loader called - size: {}".format(
            len(dataloader.dataset)))
        tb_dataloader = self.__tb_dataloader(split="val")
        dataiter = iter(tb_dataloader)
        images, label1, label2 = dataiter.next()
        img_grid = torchvision.utils.make_grid(images)
        writer = SummaryWriter('tb_photo')
        writer.add_image('Cholec80_validation_images', img_grid)
        return dataloader
    ...
    def test_dataloader(self):
        ...
        logging.info("test data loader called  - size: {}".format(
            len(dataloader.dataset)))
        tb_dataloader = self.__tb_dataloader(split="test")
        dataiter = iter(tb_dataloader)
        images, label1, label2 = dataiter.next()
        img_grid = torchvision.utils.make_grid(images)
        writer = SummaryWriter('tb_photo')
        writer.add_image('Cholec80_test_images', img_grid)
