...
from utils.accuracy_metrics import AccuracyStages, RecallOverClasse, PrecisionOverClasses
from torch import nn
import numpy as np

class IUNet(LightningModule):
    ...
    def log_average_precision_recall(self, outputs, step="val"):
        ...
        phase_avg_precision = [torch.mean(x[~x[n].isnan(), n]) for n in range(x.shape[0])]
        #phase_avg_recall = [torch.mean(y[~y[:, n].isnan(), n]) for n in range(x.shape[1])]
        phase_avg_recall = [torch.mean(y[~y[n].isnan(), n]) for n in range(x.shape[0])]
